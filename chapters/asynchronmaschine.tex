...
\emphbox{$s = \frac{n_\text{syn} - n}{n_\text{syn}} = \frac{\omega_\text{syn} - \omega_m}{\omega_\text{syn}} = \frac{\omega_1 - p\cdot\omega_m}{\omega_1} = \frac{\omega_2}{\omega_1}$}
...
\subsubsection{Phase}
\emphbox{ASM immer induktiv $\Rightarrow \varphi > 0$}
\begin{align*}
...
\emphbox{\[\underline{I}_1 = \frac{\underline{U}_1}{\omega_1 L_1} \cdot \frac{\rho_2 +js}{\rho_1\cdot\rho_2 -\sigma\cdot s+j(\rho_2 +s\cdot \rho_1)}\]
}
Anlaufstrom:\\
...
\symbolbox{bei $R_1 = 0\qquad\qquad\qquad\tan(\mu) = s_K$}
\cookbox{Stromortskurve $R_1 = 0\wedge R_\text{Fe} = 0$}{
...
\cookbox{Schlupfgerade $R_1 = 0\wedge R_\text{Fe}\neq 0$}{
...
\end{tabular}
\symbolbox{Definition Punkt D: Orthogonale Projektion von $P$ auf Im-Achse\\
...
\emphbox{$M_K\sim \left(\frac{U_1}{f_1}\right)^2\qquad M_N\sim\Phi_\delta\frac{U_1}{f_1}$}
\begin{align*}
M_i = M_R + M_L + J\frac{\partial\omega}{\partial t}
\end{align*}
\subsubsection{Drehmomentgleichung}
\emphbox{\[M_i = 3p(1-\sigma)\frac{{U_1}^2}{{\omega_1}^2 L_\sigma}\frac{s \cdot s_K}{\Delta\rho_1{s_K}^2 + 2\frac{\rho_1}{\sigma}(1-\sigma)s_K s+\Delta\rho_1 s^2}\]
}
Kippmoment:\\
...
\emphbox{\[\frac{M_i}{M_K} = \frac{2\cdot s_K\cdot s}{{s_K}^2 + s^2}\]}
