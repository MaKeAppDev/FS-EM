...
\draw [name path=poline, thick, black!30!white] (Iki1) -- (I01);

\draw [name path=sline, thick, green!70!blue]  ($(Iki1)!-2!(Pun)$) ++ (-1,0) -- ($(Iki1)!3!(Pun) +(-1,0)$);
\draw [name path=s2line, thick, green!70!blue]  (Iki1)  -- ($(Iki1)!3!(Pun)$) -- ($(Iki1)!-2!(Pun)$);
...
\path [name intersections={of=sline and poline, by=s0}];
...
\draw (Ia1) +(0.2, 0.2) node {$P_A$}
			+(0,0) node {\textbullet};
\draw (Pun)+(0, -0.25) node [right] {$P_\infty$}
			+(0,0) node {\textbullet};
\draw (Iki1)+(0, -0.25) node [right] {$S_0$}
			+(0,0) node {\textbullet};
...
\draw ($(Iki1)!-2!(Pun)$) ++ (-1,0) node [left] {Schlupfgerade}; 		
\end{scope}
\end{tikzpicture}
%!tikz source end
